...
class SubtitleDetect:
    ...
    def find_subtitle_frame_no(self, sub_remover=None, status_file_path: str = None):
        ...
        batch_size = config.MAX_BATCH_SIZE if ocr_use_gpu else 5
        frame_batch = []
        frame_numbers = []
        current_frame_no = 0
...
class SubtitleRemover:
    ...
    def propainter_mode_multi_gpu(self, tbar):
        ...
        # Track which interval is owned by which GPU to prevent duplicate dispatch
        interval_to_gpu = {}
        ...
        for i in range(min(num_workers, len(tasks_to_dispatch))):
            ...
            # Add interval to GPU mapping
            interval_to_gpu[task['interval_idx']] = gpu_id
        ...
        while True:
            ...
            try:
                ...
                owner_gpu = interval_to_gpu.get(interval_idx)
                if owner_gpu is not None and owner_gpu != gpu_id:
                    print(f"[Dispatcher] Discarding duplicate result for interval {interval_idx} from GPU {gpu_id}; current owner is GPU {owner_gpu}.")
                    idle_poll_cycles = 0
                    continue
                if interval_idx not in active_intervals:
                    print(f"[Dispatcher] Interval {interval_idx} already finalized. Ignoring late result from GPU {gpu_id}.")
                    idle_poll_cycles = 0
                    continue
                ...
                idle_poll_cycles = 0
                # mark completion for this interval and free the GPU
                active_intervals.discard(interval_idx)
                if interval_idx in interval_to_gpu:
                    del interval_to_gpu[interval_idx]
                ...
                if outstanding != 0 and not tasks_to_dispatch and (idle_poll_cycles >= 2 or lost_intervals):
                    ...
                    for i, worker in enumerate(workers):
                        ...
                        if assigned_by_gpu[gpu_id] is not None and not worker.is_alive():
                            lost_intervals.append(assigned_by_gpu[gpu_id])
                            dead_gpu_ids.append(gpu_id)
                    if lost_intervals:
                        print(f"[Dispatcher] Detected dead workers: GPUs {dead_gpu_ids} with intervals {lost_intervals}. Reassigning...")
                        try:
                            # Build list of available GPUs (alive and idle), sorted by free memory desc
                            free_list = []
                            ...
                            if not free_list:
                                print("[Dispatcher] No idle alive GPUs available to reassign right now.")
                            else:
                                free_list.sort(reverse=True)
                                # Assign as many lost intervals as we have idle GPUs
                                ...
                                for k in range(num_to_assign):
                                    ...
                                    task = task_map[interval_k]
                                    # Clear old owner and set new ownership
                                    # remove mapping from previous dead GPU if present
                                    interval_to_gpu[interval_k] = best_gpu
                                    # mark all dead gpu ownerships cleared once
        ...
        print("[Dispatcher] Sending stop signals to workers.")
        for i in range(num_workers):
            task_queues[i].put(None)

        for i, worker in enumerate(workers):
            worker.join(timeout=10)
            print(f"[Dispatcher] Worker {i} (GPU {config.DEVICES[i].index}) join returned. Alive={worker.is_alive()}")
    ...
    def sttn_mode_with_no_detection(self, tbar):
        ...
        sttn_video_inpaint(input_sub_remover=self, tbar=tbar)
    ...
    def lama_mode(self, tbar):
        ...
        while True:
            ...
            if current_frame_no not in start_end_coord_map:
                self.video_writer.write(frame)
                self.update_progress(tbar, 1)
            else:
...
class SubtitleExtractor:
    """
    Handles post-processing of raw OCR data to identify watermarks and subtitle areas.
    This class expects a raw subtitle file generated by an OCR process.
    """
    def __init__(self, video_path: str, raw_subtitle_path: str = None, status_path: str = None, detected_areas: dict = None, universal_box: list = None):
        self.video_path = video_path
        video_cap = cv2.VideoCapture(video_path)
        self.frame_count = int(video_cap.get(cv2.CAP_PROP_FRAME_COUNT) + 0.5)
        self.fps = video_cap.get(cv2.CAP_PROP_FPS)
        self.frame_width = int(video_cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        self.frame_height = int(video_cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
        self.detected_areas = detected_areas
        self.universal_box = universal_box
        video_cap.release()
    ...
    def generate_subtitle_file(self):
        ...
        """
        if not self.raw_subtitle_path or not os.path.exists(self.raw_subtitle_path) or os.path.getsize(self.raw_subtitle_path) == 0:
            print("No subtitles to generate.")
            srt_filename = os.path.join(os.path.splitext(self.video_path)[0] + '.srt')
            if not os.path.exists(os.path.dirname(srt_filename)):
    ...
    def _unite_coordinates(self, coordinates_list):
        """
        Given a list of coordinates, unifies similar coordinates into a single value.
        e.g., (255,123,456,789) and (253,122,456,799) could be unified.
        """
        # This is a placeholder for a more efficient implementation.
        # The original O(n^2) approach can be slow for long videos.
        for i, coord1 in enumerate(coordinates_list):
            for j, coord2 in enumerate(coordinates_list[i+1:], i+1):
                if SubtitleDetect.are_similar(coord1, coord2):
                    coordinates_list[j] = coord1
